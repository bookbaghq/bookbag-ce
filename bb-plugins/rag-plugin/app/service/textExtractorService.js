 ...
 * - Plain text (.txt, .md)
 * - PDF (.pdf) - via pdf-parse v2 (lazy-loaded)
 ...
 * - PDF parser lazy-loaded with proper resource cleanup
 *
 * Future (Phase 2+):
...
class TextExtractorService {
    ...
    /**
     * Lazy-load the pdf-parse module (v2 API)
     * @returns {Promise<Object>} - The PDFParse class
     */
    async loadPdfParser() {
        if (this.pdfParser) {
    ...
    /**
     * Extract text from PDF files using pdf-parse v2 API
     * @param {string} filePath - Path to PDF file
     * @returns {Promise<string>} - Extracted text from all pages
     */
    async extractPDF(filePath) {
        // Lazy-load pdf-parse PDFParse class
