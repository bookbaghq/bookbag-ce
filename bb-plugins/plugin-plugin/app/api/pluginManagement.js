...
class PluginManagementController {
  ...
  async triggerRebuild() {
    try {
      ...
      console.log('  Step 1: Regenerating plugin registry...');

      // Registry will be regenerated on next server restart via pluginLoader
      // For now, trigger it manually
      const pluginLoader = master.requestList.pluginLoader;
      if (pluginLoader) {
        const registryGenerator = require('../core/registryGenerator');
      ...
      // Note: This will take time, but we return immediately
      // In production, consider using a job queue
      const buildCommand = `cd ${nextjsDir} && npm run build`;
      ...
      exec(buildCommand, (error, stdout, stderr) => {
        ...
        // TODO: Restart Next.js server here if using pm2
        // exec('pm2 restart bookbag-frontend', (err) => { ... });
      });

      return {
        success: true,
        message: 'Rebuild process started in background',
        note: 'Build will complete in a few moments. Check server logs for status.'
      };
