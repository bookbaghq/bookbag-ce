...
export default function MediaPage() {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/bb-media/api/media/upload`, {
        method: 'POST',
        body: formData,
        credentials: 'include'
      });
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="border rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-sm font-medium text-muted-foreground">Admin Uploads</h3>
          </div>
          <p className="text-2xl font-bold">{stats?.bySource?.admin || 0}</p>
        </div>

        <div className="border rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <Upload className="w-5 h-5 text-muted-foreground" />
            <h3 className="text-sm font-medium text-muted-foreground">Client Uploads</h3>
          </div>
          <p className="text-2xl font-bold">{stats?.bySource?.client || 0}</p>
        </div>
      </div>
