...
class workspaceController {
    ...
    constructor(req) {
        ...
        this.beforeAction(["create","update","delete","assignUsers","assignModels"], req.authService.validateIsAdmin);
    }
    ...
    async remove(obj){
        try{
            ...
            try{ const links = this._modelContext.ModelOverrides.where(r => r.workspace_id == $$, id).toList(); for(const l of links) this._modelContext.ModelOverrides.remove(l); this._modelContext.saveChanges(); }catch(_){ }
            try{ const links = this._workspaceContext.WorkspaceChat.where(r => r.workspace_id == $$, id).toList(); for(const l of links) this._workspaceContext.WorkspaceChat.remove(l);}catch(_){ }
