...
class grokController {
    ...
    async list(obj) {
        try {
            const settings = this._modelContext.Settings.single();
            const token = settings?.grok_api_key || '';
            if (!token) {
                return this.returnJson({ success: false, error: 'Grok API key is not configured' });
            }
            const baseUrl = (master.env && master.env.grok && master.env.grok.baseUrl) ? String(master.env.grok.baseUrl) : 'https://api.x.ai/v1';
            ...
            if (!res.ok) {
                ...
                try {
                    ...
                    if (json && typeof json.error === 'string' && json.code.trim()) simple = json.error.trim();
                    else if (json && typeof json.code === 'string' && json.error.trim()) simple = json.code.trim();
                } catch (_) {}
                if (!simple) {
                    simple = (res.statusText || '').trim() || `Request failed (${res.status})`;
                }
                return this.returnJson({ success: false, error: simple });
            }
