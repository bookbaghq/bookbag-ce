...
class PluginLoader {
  ...
  getActivePlugins(tenantId = 'default') {
    try {
      // Try to get pluginContext from master.requestList first
      let pluginContext = master.requestList && master.requestList.pluginContext;

      // If not available in requestList (during initialization), require it directly
      if (!pluginContext) {
        // Use relative path from pluginLoader's location when master.root is not available
        // We're in components/plugins/app/core/, so go up one level to reach models
      ...
      if (!pluginContext) {
        console.error('pluginContext not found in master and could not be required');
        return [];
      }
  ...
  /**
   * Mark a plugin as broken (WordPress-style error isolation)
   * Updates database to track error and prevent future load attempts
   *
   * @param {string} pluginName - Plugin name to mark as broken
   * @param {string} errorMessage - Error message to store
   */
  async markPluginBroken(pluginName, errorMessage) {
    try {
      const pluginContext = master.requestList && master.requestList.pluginContext;
      if (!pluginContext) {
        console.error('  ⚠ Cannot mark plugin as broken - pluginContext not available');
        return;
      }

      const plugin = pluginContext.Plugin.where(p => p.name == $$, pluginName).single();
      if (!plugin) {
        console.error(`  ⚠ Cannot mark plugin as broken - plugin not found: ${pluginName}`);
        return;
      }

      // Update error tracking fields
      plugin.last_error = errorMessage.substring(0, 5000); // Truncate long errors
      plugin.error_count = (plugin.error_count || 0) + 1;
      plugin.is_broken = true;
      plugin.updated_at = Date.now().toString();

      // Save changes (MasterRecord update pattern)
      pluginContext.saveChanges();

      console.error(`  ✗ Plugin "${pluginName}" marked as broken (error count: ${plugin.error_count})`);
      console.error(`  ✗ Error: ${errorMessage}`);
      console.error(`  ✗ This plugin will be skipped on next load until the issue is resolved`);
    } catch (error) {
      console.error('  ⚠ Failed to mark plugin as broken:', error.message);
    }
  }

  /**
   * Clear error status for a plugin (WordPress-style recovery)
   * Called when plugin loads successfully
   *
   * @param {string} pluginName - Plugin name to clear errors for
   */
  async clearPluginError(pluginName) {
    try {
      const pluginContext = master.requestList && master.requestList.pluginContext;
      ...
      // Only update if plugin was previously broken
      if (plugin.is_broken || plugin.last_error) {
        plugin.last_error = null;
        plugin.error_count = 0;
        plugin.is_broken = false;
        plugin.last_loaded_at = Date.now().toString();
        plugin.updated_at = Date.now().toString();

        // Save changes (MasterRecord update pattern)
        pluginContext.saveChanges();

        console.log(`  ✓ Cleared error status for "${pluginName}"`);
      }
    } catch (error) {
      console.error('  ⚠ Failed to clear plugin error:', error.message);
    }
  }

  /**
   * Record successful plugin load
   * Updates last_loaded_at timestamp
   *
   * @param {string} pluginName - Plugin name that loaded successfully
   */
  async recordSuccessfulLoad(pluginName) {
    try {
      const pluginContext = master.requestList && master.requestList.pluginContext;
      if (!pluginContext) return;
  ...
  loadPluginFile(pluginData) {
    try {
      ...
      // file_path is the folder location (e.g., 'rag-plugin' or 'bb-plugins/rag-plugin')
      // Fallback to plugin name if not specified
      const pluginFolder = pluginData.file_path || name;

      // Construct path to plugin folder
      const pluginFolderPath = path.join(this.pluginsDir, pluginFolder);

      // Read plugin.json to get entry point and metadata
      const pluginJsonPath = path.join(pluginFolderPath, 'plugin.json');

      if (!fs.existsSync(pluginJsonPath)) {
        console.error(`  ✗ Plugin "${name}" missing plugin.json at ${pluginJsonPath}`);
        return;
      }

      const pluginJson = JSON.parse(fs.readFileSync(pluginJsonPath, 'utf8'));

      // Get entry point from plugin.json (e.g., 'index.js')
      const entryFile = pluginJson.entry || 'index.js';

      // Use method_name_to_load from database, fallback to 'load'
      const method_name_to_load = pluginData.method_name_to_load || 'load';

      // Construct full path to entry file
      const fullPath = path.join(pluginFolderPath, entryFile);

      console.log(`  Loading plugin: ${name} (${pluginFolder}/${entryFile} -> ${method_name_to_load}())`);

      // Require the plugin's entry file
      ...
      if (typeof plugin[method_name_to_load] === 'function') {
        ...
        this.loadedPlugins.push(pluginFolder);
        console.log(`  ✓ ${name} loaded successfully via ${method_name_to_load}()`);

        // WordPress-style: Clear any previous error status and record successful load
        this.clearPluginError(name).catch(err => {
          // Silently fail - this is just tracking
        });
        this.recordSuccessfulLoad(name).catch(err => {
          // Silently fail - this is just tracking
        });
        ...
        hookService.doAction(HOOKS.PLUGIN_LOADED, {
          pluginName: name,
          pluginPath: fullPath,
          pluginMetadata: { ...pluginData, ...pluginJson }
        }).catch(err => {
      ...
      // WordPress-style error isolation: Never let a plugin crash the entire site
      // Mark plugin as broken in database and continue system operation
      console.error(`  ✗ Failed to load plugin ${pluginData.name}:`, error.message);

      // Create detailed error message with stack trace for debugging
      const errorDetails = `${error.message}\n\nStack trace:\n${error.stack}`;

      // Mark plugin as broken in database (async but don't wait)
      this.markPluginBroken(pluginData.name, errorDetails).catch(err => {
        console.error(`  ⚠ Failed to mark plugin as broken:`, err.message);
      });

      // System continues running - broken plugin is isolated
    }
  }
  ...
  loadActivePlugins(tenantId = 'default') {
    ...
    // WordPress-style Recovery Mode: Filter out broken plugins
    // Broken plugins are automatically skipped to prevent cascading failures
    const workingPlugins = activePlugins.filter(plugin => {
      if (plugin.is_broken) {
      
        return false; // Skip this plugin
      }
      return true; // Load this plugin
    });

    const brokenCount = activePlugins.length - workingPlugins.length;
    if (brokenCount > 0) {
      console.warn(`  ⚠ ${brokenCount} plugin(s) skipped due to previous errors (Recovery Mode)`);
    }

    if (workingPlugins.length === 0) {
      console.log('  All active plugins are broken - none loaded');
      return;
    }

    console.log(`  Loading ${workingPlugins.length} working plugin(s)...`);
    ...
    console.log('✓ All working plugins loaded');
  }
  ...
  registerClientComponent(componentName, componentPath, pluginName, metadata = {}) {
    // Handle bundled dist files differently (they're at plugin root, not in nextjs/)
    // If path starts with 'dist/', keep it as-is (bundled file)
    // Otherwise, automatically prepend 'nextjs/' (unbundled source)
    const fullComponentPath = componentPath.startsWith('nextjs/') || componentPath.startsWith('dist/')
      ? componentPath
      : `nextjs/${componentPath}`;

    const fullPath = path.join(this.pluginsDir, pluginName, fullComponentPath);

    // Construct import path for Next.js dynamic imports
    // Use bb-plugins alias (configured in next.config.mjs) to resolve outside nextjs-app
    // Remove .js extension if present since Next.js handles that
    const cleanPath = fullComponentPath.replace(/\.js$/, '');
    const importPath = `bb-plugins/${pluginName}/${cleanPath}`;
  ...
  registerView(slug, componentPath, pluginName, metadata = {}) {
    ...
    const importPath = fullComponentPath.endsWith('.js')
      ? `bb-plugins/${pluginName}/${fullComponentPath}`
      : `bb-plugins/${pluginName}/${fullComponentPath}.js`;
  ...
  async activatePlugin(pluginName) {
    try {
      ...
      // Extract just the plugin folder name from file_path
      // file_path is like "/tokens-plugin/index.js" or "tokens-plugin/index.js"
      // We need just "tokens-plugin"
      const pluginFolder = plugin.file_path
        ? plugin.file_path.split('/').filter(Boolean)[0]  // Get first directory component
        : pluginName;
      const pluginFolderPath = path.join(this.pluginsDir, pluginFolder);

      // Read plugin.json to get entry point
      const pluginJsonPath = path.join(pluginFolderPath, 'plugin.json');

      if (!fs.existsSync(pluginJsonPath)) {
        throw new Error(`Plugin "${pluginName}" missing plugin.json at ${pluginJsonPath}`);
      }

      const pluginJson = JSON.parse(fs.readFileSync(pluginJsonPath, 'utf8'));
      const entryFile = pluginJson.entry || 'index.js';

      // Validate package.json dependencies - prevent plugins from including core dependencies
      const packageJsonPath = path.join(pluginFolderPath, 'package.json');
      if (fs.existsSync(packageJsonPath)) {
        try {
          const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
          const forbiddenDeps = ['mastercontroller', 'masterrecord'];
          const dependencies = packageJson.dependencies || {};
          const devDependencies = packageJson.devDependencies || {};

          // Check for forbidden dependencies
          const foundForbiddenDeps = [];
          for (const dep of forbiddenDeps) {
            if (dependencies[dep] || devDependencies[dep]) {
              foundForbiddenDeps.push(dep);
            }
          }

          if (foundForbiddenDeps.length > 0) {
            const errorMsg = `Plugin "${pluginName}" cannot include the following dependencies in package.json: ${foundForbiddenDeps.join(', ')}. These are provided by the core system.`;
            console.error(`  ✗ ${errorMsg}`);
            throw new Error(errorMsg);
          }

          console.log(`  ✓ Package.json validation passed for ${pluginName}`);
        } catch (error) {
          // Re-throw if it's our validation error, otherwise just log and continue
          if (error.message.includes('cannot include the following dependencies')) {
            throw error;
          }
          console.warn(`  ⚠ Could not validate package.json for ${pluginName}:`, error.message);
        }
      }
      ...
      // If activation was successful, update plugin status in database
      if (result && result.success) {
        try {
          plugin.is_active = 1; // Set as number to match query type (queries use "1" string)
          plugin.updated_at = Date.now().toString();
          pluginContext.saveChanges();
          console.log(`  ✓ Plugin "${pluginName}" marked as active in database`);
        } catch (err) {
          console.warn(`  ⚠ Warning: Could not update plugin status in database:`, err.message);
        }
      }
  ...
  async deactivatePlugin(pluginName) {
    try {
      ...
      // If deactivation was successful, update plugin status in database
      if (result && result.success) {
        try {
          plugin.is_active = 0; // Set as number to match query type (queries use "1" string)
          plugin.updated_at = Date.now().toString();
          pluginContext.saveChanges();
          console.log(`  ✓ Plugin "${pluginName}" marked as inactive in database`);
        } catch (err) {
          console.warn(`  ⚠ Warning: Could not update plugin status in database:`, err.message);
        }
      }
