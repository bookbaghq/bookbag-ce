...
class pluginComponentsController {
  ...
  async listComponents(req, res) {
    try {
      ...
      // Map to clean objects to avoid circular references and dirty data
      const map = {
        'name': 'name',
      ...
      const cleanComponents = Array.isArray(components)
        ? components.map(item => mapObject(item, map))
        : [];
  ...
  async getComponent(req, res) {
    try {
      ...
      const cleanComponent = mapObject({name, ...component}, map);
      cleanComponent.name = name; // Ensure name is included
