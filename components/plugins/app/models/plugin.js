...
class Plugin {
  ...
  // WordPress-style error handling fields
  // Last error message (if plugin failed to load)
  last_error(db) {
    db.string().nullable();
  }

  // Error count (how many times plugin has failed)
  error_count(db) {
  ...
  // Whether plugin is temporarily disabled due to errors
  is_broken(db) {
